...
| 编号 | 状态 | 工作项 |
|------|------|--------|
| A-01 | ✅ | `DataForge.Core.Parquet` — FromParquet / ToParquetAsync |
| A-02 | ✅ | `DataForge.Core.DuckDB` — FromDuckDb / ToDuckDbAsync |
| A-03 | ✅ | `DataForge.SchemaInfer` CLI — csv/json → validate.rules |
| A-04 | ⬜ | 国产库 PoC（达梦 / 人大金仓，评估后选一家） |
...
| DEC-03 | ✅ | `tools/DataForge.Sync` v0.5：全源汇 + validate + watch + parquet/duckdb |
| A-01/A-02 | ✅ | Parquet + DuckDB 扩展包 |
| A-03 | ✅ | `tools/DataForge.SchemaInfer` |
| D-01~D-08 | 🟡 | RowError、WithBadRowOutput、SelectParallelAsync、DI 包、ToDataForge 已完成 |
